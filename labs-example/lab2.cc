...
//
  ...
  NS_LOG_INFO ("Enable All");
  ns3::LogComponentEnableAll (LOG_LEVEL_INFO);
  ...
  Ptr<PcapFileWrapper> file = pcapHelper.CreateFile ("lab2_trace.pcap", std::ios::out, PcapHelper::DLT_PPP);

  // pcapHelper.EnablePcapAll ("lab2.pcap", true)
  // Obtendo dados do Trace de PhyRxDrop dos Devices 
//   c.Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop,file));
  c.Get (1)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop,file));
//   c.Get (2)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop,file));
//   c.Get (3)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop,file));
//   c.Get (4)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop,file));
//   c.Get (5)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop,file));
...
// Increase UDP Rate
  ...
  //Gerar animação
  AnimationInterface anim ("lab2_animation.xml"); // Mandatory
  // Adicionando configurações em todos os nós
  for (uint32_t i = 0; i < c.GetN(); ++i){
    anim.UpdateNodeDescription (c.Get(i), "Node"); // Optional
    anim.UpdateNodeColor (c.Get(i), 255, 0, 0); // Coloração
  ...
  if (enableFlowMonitor)
    {
	  ...
	  flowmon->SerializeToXmlFile("lab_2_flowmon.xml", true, true);
    }
  Simulator::Destroy ();
  NS_LOG_INFO ("Done.");
}