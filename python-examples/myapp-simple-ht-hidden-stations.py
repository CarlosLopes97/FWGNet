...
def compare(app_protocol):
    ...
    if app_protocol == "udp" or app_protocol == "tcp" or app_protocol == "802_11":
        Applications = ["Send"]
...
def ch_802_11_read(app_protocol):
    global const_Size
    global validation
    ns3_df = pd.read_csv("scratch/"+app_protocol+"_trace.txt", sep=";", names=["Time", "Size"])
    # print(ns3_df[:10])
...
def main(argv):
    ...
    if t_net == "adhoc":
        ...
        allNodes = ns3.NodeContainer()
        # Criando nós
        allNodes.Create(nUsers)

    if t_net == "ap_sta":
        wifiStaNodes = ns.network.NodeContainer ()
        wifiStaNodes.Create (nUsers)
    ...
    if t_net == "adhoc":
        ...
        mobility.SetPositionAllocator ("ns3::RandomRectanglePositionAllocator",
                            "X", ns.core.StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=1]"),
                            "Y", ns.core.StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=1]"))
    ...
    wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager", "DataMode", ns.core.StringValue ("ErpOfdmRate6Mbps"), 
                                                                  "ControlMode", ns.core.StringValue ("ErpOfdmRate6Mbps"))
    ...
    if (app_protocol == "http"):
        if validation == "True":
            ...
            sinkApps = packetSinkHelper.Install(allNodes.Get(0))
            
            sinkApps.Start(ns.core.Seconds(0.0))
            sinkApps.Stop(ns.core.Seconds(timeStopSimulation))
            # Client Application
            sinkAddress = ns.network.Address(ns.network.InetSocketAddress(AllInterface.GetAddress(0), sinkPort))
            ns3TcpSocket = ns.network.Socket.CreateSocket(allNodes.Get(1), ns.internet.TcpSocketFactory.GetTypeId())
            app = ReqMyApp()
            # def Setup(self, socket, address, packetSize, nRequestPackets, dataRate):
            ...
            sinkApps1 = packetSinkHelper1.Install(allNodes.Get(1))
            
            sinkApps1.Start(ns.core.Seconds(0.001))
            sinkApps1.Stop(ns.core.Seconds(timeStopSimulation))
            # Client Application
            sinkAddress1 = ns.network.Address(ns.network.InetSocketAddress(AllInterface.GetAddress(1), sinkPort1))
            ns3TcpSocket1 = ns.network.Socket.CreateSocket(allNodes.Get(0), ns.internet.TcpSocketFactory.GetTypeId())
            app1 = RespMyApp()
            # def Setup(self, socket, address, packetSize, nRequestPackets, dataRate):
            # app.Setup(ns3TcpSocket, sinkAddress, packetSize, nRequestPackets, ns3.DataRate(dataRate))
            ...
            for i in range(0,len(req_sinkApps),1):
                ...
                req_sinkApps[i] = req_packetSinkHelper[i].Install(allNodes.Get(i))
            ...
            for i in range(0,len(req_app),1):
                req_sinkAddress = ns.network.Address(ns.network.InetSocketAddress(AllInterface.GetAddress(0), req_sinkPort))
                req_ns3TcpSocket = ns.network.Socket.CreateSocket(allNodes.Get(i), ns.internet.TcpSocketFactory.GetTypeId())
                req_app[i] = ReqMyApp()
                # def Setup(self, socket, address, packetSize, nRequestPackets, dataRate):
                # app.Setup(ns3TcpSocket, sinkAddress, packetSize, nRequestPackets, ns3.DataRate(dataRate))
                req_app[i].Setup(req_ns3TcpSocket, req_sinkAddress, nRequestPackets)
                allNodes.Get(i+1).AddApplication(req_app[i])
            ...
            for i in range(0,len(resp_sinkApps),1):
                ...
                resp_sinkApps[i] = resp_packetSinkHelper[i].Install(allNodes.Get(i))
            ...
            for i in range(0,len(resp_app),1):
                resp_sinkAddress = ns.network.Address(ns.network.InetSocketAddress(AllInterface.GetAddress(i), resp_sinkPort))
                resp_ns3TcpSocket = ns.network.Socket.CreateSocket(allNodes.Get(0), ns.internet.TcpSocketFactory.GetTypeId())
                ...
                allNodes.Get(i+1).AddApplication(resp_app[i])
    ...
    if (app_protocol == "ftp"):
        if validation == "True":
            ...
            sinkApps0 = packetSinkHelper0.Install(allNodes.Get(0))
            
            sinkApps0.Start(ns.core.Seconds(0.0))
            # sinkApps0.Stop(ns.core.Seconds(timeStopSimulation))
            # Client Application
            sinkAddress0 = ns.network.Address(ns.network.InetSocketAddress(AllInterface.GetAddress(0), sinkPort0))
            ns3TcpSocket0 = ns.network.Socket.CreateSocket(allNodes.Get(1), ns.internet.TcpSocketFactory.GetTypeId())
            app0 = ReqMyApp()
            # def Setup(self, socket, address, packetSize, nRequestPackets, dataRate):
            # app.Setup(ns3TcpSocket, sinkAddress, packetSize, nRequestPackets, ns3.DataRate(dataRate))
            app0.Setup(ns3TcpSocket0, sinkAddress0, nRequestPackets)
            allNodes.Get(1).AddApplication(app0)
            ...
            sinkApps = packetSinkHelper.Install(allNodes.Get(1))
            
            sinkApps.Start(ns.core.Seconds(0.02))
            # sinkApps.Stop(ns.core.Seconds(timeStopSimulation))
            # Client Application
            sinkAddress = ns.network.Address(ns.network.InetSocketAddress(AllInterface.GetAddress(1), sinkPort))
            ns3TcpSocket = ns.network.Socket.CreateSocket(allNodes.Get(0), ns.internet.TcpSocketFactory.GetTypeId())
            ...
            for i in range(0,len(sinkApps),1):
                ...
                sinkApps[i] = packetSinkHelper[i].Install(allNodes.Get(i))
            ...
            for i in range(0,len(app),1):
                ...
                sinkAddress = ns.network.Address(ns.network.InetSocketAddress(AllInterface.GetAddress(i), sinkPort))
                ns3UdpSocket = ns.network.Socket.CreateSocket(allNodes.Get(0), ns.internet.TcpSocketFactory.GetTypeId())
                ...
                allNodes.Get(i+1).AddApplication(app[i])
                # Inicio da aplicação
    ...
    if (app_protocol == "udp"):
        ...
        if validation == "True":
            ...
            # Aplicação do cliente
            sinkAddress = ns.network.Address(ns.network.InetSocketAddress(AllInterface.GetAddress(1), sinkPort))
            ns3UdpSocket = ns.network.Socket.CreateSocket(allNodes.Get(0), ns.internet.UdpSocketFactory.GetTypeId())
            # Definindo aplicação na classe Myapp
            app = MyApp()
    ...
    if (app_protocol == "802_11"):
        ...
        if validation == "True":
            timeStopSimulation, nPackets = ch_802_11_read(app_protocol)
            # Application UDP
            sinkPort = 8080
            # Aplicação do servidor
            packetSinkHelper = ns.applications.PacketSinkHelper("ns3::UdpSocketFactory", ns.network.InetSocketAddress(ns.network.Ipv4Address.GetAny(), sinkPort))
    ...
    phy.EnablePcapAll("../../../Results/Traces/"+app_protocol+"_wifi-myapp-py.pcap", False)

    # std::ios::out, ns3.PcapHelper.DLT_PPP
